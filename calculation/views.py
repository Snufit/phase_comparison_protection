...
from .services import PowerFactoryManager, TopologyAnalysisService, SettingsCalculationService, \
...
FAULT_TYPE_COLORS = {
    'К(3)': {"backgroundColor": "rgba(54, 162, 235, 0.5)", "borderColor": "rgba(54, 162, 235, 1)"},
    'К(2)': {"backgroundColor": "rgba(54, 162, 235, 0.5)", "borderColor": "rgba(54, 162, 235, 1)"},
    'К(1,1)': {"backgroundColor": "rgba(75, 192, 192, 0.5)", "borderColor": "rgba(75, 192, 192, 1)"},
    'К(1)': {"backgroundColor": "rgba(255, 99, 132, 0.5)", "borderColor": "rgba(255, 99, 132, 1)"}
}
...
class CalculationView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request):
        ...
        if line_id:
            ...
            line_form = LineSelectionForm(initial={'line': line.id})
        ...
        if half_set1_topology and half_set2_topology and half_set1_topology_display and half_set2_topology_display:
            ...
            submodes_form1 = SubmodesConfigurationForm(
                prefix='half_set1',
                initial={'max_outages': half_set1_max_outages, 'max_lines': half_set1_max_lines, 'max_autotransformers': half_set1_max_autotransformers},
            )
            submodes_form2 = SubmodesConfigurationForm(
                prefix='half_set2',
                initial={'max_outages': half_set2_max_outages, 'max_lines': half_set2_max_lines, 'max_autotransformers': half_set2_max_autotransformers},
            )
...
class TestView(View):
    ...
    def get(self, request):
        ...
        coefficients = request.session.get('coefficients', {'coefficient1': 1.3, 'coefficient2': 1.2})
...
def sensitivity_chart_view(request):
    ...
    for fault_type in fault_types:
        color = FAULT_TYPE_COLORS.get(fault_type, {"backgroundColor": "rgba(201, 203, 207, 0.5)", "borderColor": "rgba(201, 203, 207, 1)"})
        datasets.append({
            "label": fault_type,
            "data": [grouped_data[topology].get(fault_type, 0) for topology in labels],
...
def export_sensitivity_analysis(request, calculation_meta_id):
    ...
    for row_num, result in enumerate(sens_analysis, start=2):
        ws.cell(row=row_num, column=1, value=str(result.settings_calculation.protection_half_set))
    ...
    response["Content-Disposition"] = f'attachment; filename="sensitivity_analysis_{calculation_meta.id}.xlsx"'
    wb.save(response)
...
def export_calculation_results(request, calculation_meta_id):
    calculation_meta = CalculationMeta.objects.get(id=calculation_meta_id)
    calculations = SettingsCalculation.objects.filter(
        calculation_meta=calculation_meta
    )
    ...
    # Заголовки
    headers = ["Полукомплект", "Орган ДФЗ", "Коэффициенты", "Значение уставки"]
    for col_num, header in enumerate(headers, 1):
        cell = ws.cell(row=1, column=col_num, value=header)
        cell.font = Font(bold=True)
    ...
    for row_num, calculation in enumerate(calculations, start=2):
        coefficients = (
            "\n".join(f"{key}: {value}" for key, value in calculation.calculation_factors.items())
            if calculation.calculation_factors else "Нет коэффициентов"
        )
    ...
    response = HttpResponse(content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    response["Content-Disposition"] = f'attachment; filename="calculation_results_{calculation_meta.id}.xlsx"'
