...
from core.models import (
    Line, LineType, LineBranch, Substation, 
    ProtectionHalfSet, ProtectionDevice
)
...
from calculation.services.powerfactory_locator import (
    get_line_type, _has_branches, _get_line_end_substations,
    _get_main_substations_with_voltage, get_pf_line
)
...
class ProjectSyncService:
    ...
    def needs_sync(self) -> bool:
        ...
        lines_count = Line.objects.filter(
            project_name=self.project_name,
            pf_name__isnull=False
        ).exclude(pf_name="").count()
        ...
        print(f"[DEBUG] needs_sync для проекта '{self.project_name}': lines={lines_count}, substations={substations_count}, needs={needs}")
        return needs
    ...
    def sync_project(self, force_full: bool = False) -> Dict[str, any]:
        ...
        if not force_full and not needs_full_sync:
            ...
            results['message'] = "Данные проекта уже синхронизированы. Используются существующие данные."
            return results
        
        try:
            # Подключаемся к PowerFactory с указанным проектом
            self.app = self.pf_manager.get_application(project_name=self.project_name)
            ...
            print(f"[DEBUG] sync_project: needs_full_sync={needs_full_sync}, force_full={force_full}, update_only={update_only}")
            
            # 1. Импорт линий
            results['lines'] = self._import_lines(update_only=update_only)
            
            # 2. Импорт подстанций
            results['substations'] = self._import_substations(update_only=update_only)
            ...
            if force_full or results['half_sets']['created'] > 0:
                results['topology'] = self._analyze_topology(force_refresh=force_full)
            ...
            for index, pf_line in enumerate(pf_lines):
                try:
                    ...
                    if line_type_str and ("ответвлением" in line_type_str or "ответвлениями" in line_type_str):
                        try:
                            branch_substations = _has_branches(
                                self.app, pf_line, return_branch_substations=True, check_substations=True
                            )
                    ...
                    try:
                        ...
                        if update_only:
                            print(f"[DEBUG] Пропуск создания линии '{pf_name}' (update_only=True)")
                            continue
                        print(f"[DEBUG] Создание новой линии '{pf_name}' для проекта '{self.project_name}'")
                        try:
                            ...
                            created = True
                            print(f"[DEBUG] Линия '{pf_name}' успешно создана (ID: {existing_line.id})")
                        except Exception as create_error:
                            print(f"[DEBUG] Ошибка при создании линии '{pf_name}': {create_error}")
                            stats['errors'] += 1
                            continue
                    ...
                    if not created:
                        ...
                        existing_line.project_name = self.project_name  # Обновляем проект
                    ...
                    if has_branches:
                        ...
                        if existing_branches.count() < len(branch_substations):
                            for _ in range(len(branch_substations) - existing_branches.count()):
    ...
    def _import_substations(self, update_only: bool = False) -> Dict[str, int]:
        ...
        try:
            pf_substations = self.app.GetCalcRelevantObjects("*.ElmSubstat") or []
        ...
        try:
            ...
            for line in unique_lines:
                try:
                    ...
                    all_substations = _get_line_end_substations(pf_line, self.app)
                    main_substations = _get_main_substations_with_voltage(pf_line, self.app)
                    ...
                    if len(branch_substations_list) > line_branches.count():
                        for _ in range(len(branch_substations_list) - line_branches.count()):
                            LineBranch.objects.create(
                                line=line,
