...
class SubmodesConfigurationForm(forms.Form):
    ...
    min_outages = forms.IntegerField(
        ...
        required=True,
        initial=0,
        widget=forms.NumberInput(attrs={'class': 'form-control', 'min': 0, 'step': 1.0})
    )
...
class CalculationFactorsForm(forms.Form):
    ...
    il_grading_factor = forms.FloatField(
        ...
        initial=1.3,
        required=True,
        widget=forms.NumberInput(attrs={'class': 'form-control', 'min': 1.2, 'max': 2.0, 'step': 0.1})
    )
    il_reset_factor = forms.FloatField(
        label="Коэффициент возврата",
        initial=0.9,
        required=True,
        widget=forms.NumberInput(attrs={'class': 'form-control', 'min': 0.8, 'max': 1.0, 'step': 0.01})
    )
    il_matching_factor = forms.FloatField(
        label="Коэффициент согласования",
        initial=1.4,
        required=True,
        widget=forms.NumberInput(attrs={'class': 'form-control', 'min': 1.4, 'max': 2.0, 'step': 0.1})
    )
    i2_imbalance_factor = forms.FloatField(
        label="Коэффициент небаланса",
        initial=0.05,
        required=True,
        widget=forms.NumberInput(attrs={'class': 'form-control', 'min': 0.02, 'max': 0.05, 'step': 0.01})
    )
    ...
    u2_imbalance_voltage = forms.FloatField(
        ...
        initial=1.5,
        required=True,
        widget=forms.NumberInput(attrs={'class': 'form-control', 'min': 1.5, 'max': 2.0, 'step': 0.1})
    )
