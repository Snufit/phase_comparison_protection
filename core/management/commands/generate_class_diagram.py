"""
Django management command для автоматической генерации PlantUML-диаграммы классов.

Использование:
    python manage.py generate_class_diagram                    # Полная диаграмма
    python manage.py generate_class_diagram --models-only      # Только модели
    python manage.py generate_class_diagram --services-only     # Только сервисы
    python manage.py generate_class_diagram --output docs/diagram.puml  # С указанием пути
"""
...
class Command(BaseCommand):
    help = "Генерирует PlantUML-диаграмму классов из Django-моделей и сервисов"

    def add_arguments(self, parser):
        parser.add_argument(
            '--models-only',
            action='store_true',
            help='Генерировать только диаграмму Django-моделей',
        )
        parser.add_argument(
            '--services-only',
            action='store_true',
            help='Генерировать только диаграмму сервисов',
        )
        parser.add_argument(
            '--output', '-o',
            type=str,
            default='diagram.puml',
            help='Имя выходного файла (по умолчанию: diagram.puml)',
        )
    ...
    def get_class_methods(self, class_obj):
    ...
    def generate_models_diagram(self):
        ...
        for app_label, models_list in models_by_app.items():
            ...
            for model_name, model in models_list:
                ...
                for field in fields[:10]:  # Ограничиваем количество полей
                    puml.append(f"    {field}")
                if len(fields) > 10:
    ...
    def generate_services_diagram(self):
        ...
        for service_name, service_class in services:
            ...
            # Добавляем методы
            methods = self.get_class_methods(service_class)
    ...
    def generate_combined_diagram(self):
        """Генерирует комбинированную диаграмму."""
        models_diagram = self.generate_models_diagram()
        services_diagram = self.generate_services_diagram()
    ...
    def handle(self, *args, **options):
        ...
        if models_only:
            diagram = self.generate_models_diagram()
            if output_file == 'diagram.puml':
                output_file = 'models_diagram.puml'
        elif services_only:
            diagram = self.generate_services_diagram()
            if output_file == 'diagram.puml':
                output_file = 'services_diagram.puml'
        else:
            # По умолчанию - полная диаграмма
            diagram = self.generate_combined_diagram()
        ...
        self.stdout.write(
            self.style.SUCCESS(f"✓ Диаграмма сохранена в {output_file}")
        )
        self.stdout.write(
            f"  Откройте файл в PlantUML или используйте онлайн-редактор: "
            f"http://www.plantuml.com/plantuml/uml/"
        )
